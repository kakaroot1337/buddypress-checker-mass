...
def jamet1(url):
    try:
        ...
        cek = requests.get(url+"/wp-json/buddypress/v1/signup", headers=Agent1, timeout=5)
        if 'bp_rest_authorization_required' in cek.text.encode('utf-8'):
            print(ktngreen + '[Ok] Live kayanya(Buddypress - Member)' + '(' + url + ')'+ CEND)
            open('Valid-Buddypress.txt', 'a').write(url+'/wp-json/buddypress/v1/signup'+'\n')            
        else:
